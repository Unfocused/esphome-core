...
optional<float> OneEuroFilter::new_value(float value) {
  ...
  ESP_LOGVV(TAG, "OneEuroFilter(%p)::new_value(%f), frequency:%f, min_cutoff:%f, cutoff_slope:%f, derivative_cutoff:%f",
            this, value, this->frequency_, this->min_cutoff_, this->cutoff_slope_, this->derivative_cutoff_);
  return {};
}
...
OneEuroFilter::LowPassFilter(float alpha, float initial_value) : initialized_(false), alpha_(alpha), {}
...
bool OneEuroFilter::LowPassFilter::hasLastRawValue(void) { return this->initialized_; }
float OneEuroFilter::LowPassFilter::lastRawValue(void) { return this->lastRawValue_; }
